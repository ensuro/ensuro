...
variants.forEach((variant) => {
  describe(`${variant.contract} contract tests`, function () {
    ...
    it("Creates a policy if the right signature is provided", async () => {
      ...
      const policyParams = await variant.defaultPolicyParams({ rmAddress: rm.target });
      ...
      expect(policyId / twoPow96).to.be.equal(BigInt(rm.target));
      ...
      await expect(rm.connect(anon).resolvePolicy([...newPolicyEvt.args[1]], policyParams.payout)).to.be.revertedWith(
        accessControlMessage(anon.address, rm.target, "RESOLVER_ROLE")
      );
    ...
    it("Rejects a policy if signed by unauthorized user", async () => {
      ...
      await expect(variant.newPolicy(rm, cust, policyParams, cust, signature)).to.be.revertedWith(
        accessControlMessage(anon.address, rm.target, "PRICER_ROLE")
      );
    });
    ...
    it("Rejects a policy if quote expired or validUntil changed", async () => {
      ...
      const policyParams = await variant.defaultPolicyParams({ rmAddress: rm.target, validUntil: now - 1000 });
      const signature = await variant.makeSignedQuote(signer, policyParams);
      ...
      await expect(variant.newPolicy(rm, cust, policyParams, cust, signature)).to.be.revertedWith(
        accessControlMessage(recoveredAddress, rm.target, "PRICER_ROLE")
      );
    });
    ...
    it("Rejects policy creation and resolution if it's paused", async () => {
      ...
      await accessManager.grantComponentRole(rm.target, await rm.GUARDIAN_ROLE(), guardian.address);
      ...
      await accessManager.grantComponentRole(rm.target, await rm.PRICER_ROLE(), anon.address);
    ...
    it("Creates a policy where using newPolicyFull", async () => {
      const { rm, pool, currency } = await helpers.loadFixture(deployPoolFixture);
      const policyParams = await variant.defaultPolicyParams({ rmAddress: rm.target, premium: _A(200) });
      const signature = await variant.makeSignedQuote(signer, policyParams);
      ...
      ).to.be.revertedWith(accessControlMessage(anon.address, rm.target, "RESOLVER_ROLE"));
    ...
    it("If creation is not open, only authorized users can create policies", async () => {
      ...
      const signature = await variant.makeSignedQuote(signer, policyParams);
      await expect(variant.newPolicy(rm, cust, policyParams, cust, signature)).to.be.revertedWith(
        accessControlMessage(cust.address, rm.target, "POLICY_CREATOR_ROLE")
      );
      await expect(
        variant.newPolicy(rm, cust, policyParams, cust, signature, "newPolicyPaidByHolder")
      ).to.be.revertedWith(accessControlMessage(cust.address, rm.target, "POLICY_CREATOR_ROLE"));
      await expect(variant.newPolicy(rm, cust, policyParams, cust, signature, "newPolicyFull")).to.be.revertedWith(
        accessControlMessage(cust.address, rm.target, "POLICY_CREATOR_ROLE")
      );
      await accessManager.grantComponentRole(rm.target, await rm.POLICY_CREATOR_ROLE(), cust.address);
